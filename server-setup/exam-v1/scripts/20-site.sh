#!/usr/bin/env bash
# Server block nginx untuk Exam v1 (Laravel).
#
# Ditulis di sini, bukan lewat write_site_conf milik setup induk: fungsi itu
# menyetel `root /var/www/<situs>` sedangkan Laravel butuh `.../public`. Selisih
# satu direktori, tetapi akibatnya seluruh kode sumber — termasuk .env — berada
# di dalam jangkauan web.
...
laravel_body() {
  cat <<CONF
    ...
    location ~ \.php\$ {
        ...
        # Koreksi otomatis, impor soal, dan pembuatan rapor bisa memakan waktu.
...
TMP="$(mktemp)"
{
  echo "# ${APP_SITE} — dibuat oleh server-setup/exam-v1 (sudo make site / make ssl)."
