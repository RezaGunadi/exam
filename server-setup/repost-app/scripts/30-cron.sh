#!/usr/bin/env bash
# Penjadwal Laravel untuk cuandariponsel.
#
# INI BUKAN PELENGKAP. routes/console.php mendaftarkan lima perintah terjadwal,
# dan setiap satunya memindahkan UANG atau melindungi orang dari kehilangan
# uang:
#
#   submissions:auto-accept  kiriman yang tidak di-QC 4 hari otomatis diterima
#   campaigns:auto-close     sisa escrow dikembalikan ke pemberi kerja (H+7)
#   topups:expire            top-up menggantung dibatalkan tiap jam
#   submissions:check-links  postingan yang sudah dihapus ditandai lebih dulu
#   earnings:settle          pendapatan yang masa tahannya lewat dicairkan
#
# Tanpa satu baris cron ini, tidak satu pun berjalan — dan tidak ada satu pun
# pesan error yang muncul. Yang terlihat hanya saldo freelancer yang tidak
# pernah cair dan escrow pemberi kerja yang tertahan selamanya.
...
TMP="$(mktemp)"
cat > "$TMP" <<CRON
# Penjadwal Laravel ${APP_SITE} — dibuat oleh server-setup/repost-app.
#
# Laravel menjalankan SELURUH jadwalnya dari satu entri per menit; jadwal
# masing-masing perintah ditentukan di routes/console.php, bukan di sini.
#
