#!/usr/bin/env bash
# Konfigurasi & fungsi bersama untuk deploy cuandariponsel.
# Di-source oleh skrip lain di direktori ini; tidak untuk dijalankan sendiri.

REPOST_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPOST_APP_DIR="$(dirname "$REPOST_SCRIPT_DIR")"
SETUP_DIR="$(dirname "$REPOST_APP_DIR")"
...
APP_SITE="${SITE:-repost}"
APP_DOMAIN="${DOMAIN:-repost.ragh.co.id}"
APP_REPO="${REPO:-git@github.com:RezaGunadi/cuan_dari_ponsel.git}"
APP_DB="${DB_NAME:-cuandariponsel}"
...
# Keduanya memakai write_site_conf. Deteksi otomatisnya memang menemukan
...
# termasuk .env berpindah ke dalam jangkauan web.
#
...
# mengurus situs lain dan tidak tahu ia baru saja membuka .env situs ini.
assert_not_managed_by_setup() {
  ...
  # NODE_SITES juga. 40-node.sh menjalankan `npm ci && npm run build` di
  # /var/www/<nama> lalu MENULIS ULANG server block-nya lewat write_site_conf —
  # jalur yang sama, hanya lewat pintu yang berbeda.
  while read -r name; do
    if [ "$name" = "$APP_SITE" ]; then
      warn "'$APP_SITE' terdaftar di NODE_SITES pada $SETUP_DIR/.env"
      warn "  'sudo make node' di direktori induk akan menulis ulang server"
      warn "  block-nya setelah build. Build aset situs ini diurus 'make assets'."
      die "hapus '$APP_SITE' dari NODE_SITES lebih dulu"
    fi
...
# DIRINGKAS jadi satu. .env.example repo ini memuat DB_CONNECTION dua kali (satu
# untuk sqlite, satu dikomentari untuk mysql); membiarkan keduanya berarti yang
# menang ditentukan oleh urutan pembacaan dotenv, bukan oleh apa yang tertulis.
set_env_kv() {
