#!/usr/bin/env bash
# HTTPS untuk cuandariponsel lewat Let's Encrypt.
#
# Domain ini TIDAK boleh diurus 35-ssl.sh milik setup induk: skrip itu menulis
# ulang server block dengan write_site_conf, yang batas unggahan dan timeout
# FPM-nya tidak sesuai untuk unggahan video di sini. Jadi sertifikatnya diambil
# di sini, dan server block-nya tetap ditulis 20-site.sh.
#
...
#
# Berkasnya sama persis dengan yang dipasang 35-ssl.sh induk dan exam-v1, jadi
# aman bila ketiganya pernah berjalan di server yang sama.
