#!/usr/bin/env bash
# Izin berkas Laravel untuk cuandariponsel.
#
# Menerapkan resep izin yang sama dengan exam-v1:
#
...
#
# DI APLIKASI INI TARUHANNYA LEBIH TINGGI daripada di exam-v1: yang diunggah
# adalah berkas video dari pengguna anonim yang baru mendaftar, bukan jawaban
# ujian dari siswa yang identitasnya sudah diketahui. Yang menahannya adalah
# PortfolioStorage — unggahan disimpan di storage/app/private atau R2, tidak
# pernah di public/. Jaga agar tetap begitu.
#
...
# database, APP_KEY, kunci Xendit, dan kredensial R2.
...
# dan node_modules/ bersama-sama berisi lebih dari seratus ribu berkas, dan
# versi `\;` mengubah langkah ini menjadi tunggu beberapa menit tiap deploy.
...
# (pint, phpunit, dan apa pun yang dipanggil composer sebagai skrip) dan
# node_modules/.bin (vite, tailwindcss). Kegagalan yang muncul kemudian berbunyi
# "Permission denied" pada nama paket, tanpa menyinggung bahwa penyebabnya
# adalah perintah chmod di langkah deploy.
...
if [ -d "$APP_ROOT/node_modules/.bin" ]; then
  # -L: isi .bin adalah symlink ke skrip di dalam paketnya, dan chmod pada
  # symlink tidak mengubah apa pun. Yang perlu dieksekusi adalah tujuannya.
  find -L "$APP_ROOT/node_modules/.bin" -type f -exec chmod 775 {} + 2>/dev/null || true
  ok "node_modules/.bin bisa dieksekusi lagi"
fi
...
if [ -f "$APP_ENV_FILE" ]; then
  ...
  ok ".env tetap 640 (password database, Xendit, dan R2 ada di dalamnya)"
fi
...
warn "  Unggahan di aplikasi ini datang dari pengguna anonim — pastikan ia"
warn "  tetap mendarat di storage/app/private atau R2, bukan di public/."
